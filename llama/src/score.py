...
def get_samples_from_signature(model_signature_x,
    ...
    if model_signature_inputs and previous_sample_input is None:
        if model_signature_inputs.is_tensor_spec():
            sample_input_x = create_tensor_spec_sample_io(
                model_signature_inputs)
        else:
            try:
                sample_input_x = create_col_spec_sample_io(
                    model_signature_inputs)
            except:
    ...
    if model_signature_outputs and previous_sample_output is None:
        if model_signature_outputs.is_tensor_spec():
            sample_output_x = create_tensor_spec_sample_io(
...
def get_parameter_type(sample_input_ex, sample_output_ex=None):
    if sample_input_ex is None:
        ...
        try:
            ...
            #     input_param = StandardPythonParameterType(param_arg)
                ...
                param_arg = {}
                for key, value in sample_input_ex.items():
                    param_arg[key] = NumpyParameterType(value,
                                                        enforce_shape=False)
                input_param = StandardPythonParameterType(param_arg)
            else:
    ...
    if sample_output_ex is None:
        ...
        if isinstance(sample_output_ex, np.ndarray):
            # Unnamed tensor input
            output_param = NumpyParameterType(sample_output_ex,
                                              enforce_shape=False)
            ...
            output_param = PandasParameterType(sample_output_ex,
                                               enforce_shape=False,
                                               orient="records")
...
def init():
    ...
    aacs_client = ContentSafetyClient(endpoint,
    ...
    try:
        ...
        _logger.error(
            "Error initializing model_inputs collector and model_outputs collector. {}"
            .format(e))
...
def analyze_response(response):
    ...
    if response.self_harm_result is not None:
        print("SelfHarm severity: {}".format(
...
def run(input_data):
    ...
    if severity > aacs_threshold:
        return {}
    if (isinstance(input_data, np.ndarray) or
        (isinstance(input_data, dict) and input_data and
         isinstance(list(input_data.values())[0], np.ndarray)) or
        (pandas_installed and isinstance(input_data, pd.DataFrame))):
        # Collect model input
        try:
            context = inputs_collector.collect(input_data)
        except Exception as e:
            _logger.error(
                "Error collecting model_inputs collection request. {}".format(
                    e))
        ...
        try:
            ...
            _logger.error(
                "Error collecting model_outputs collection request. {}".format(
                    e))
...
def parse_model_input_from_input_data_traditional(input_data):
    ...
    elif (isinstance(input_data, dict) and "columns" in input_data and
          "index" in input_data and "data" in input_data):
        # if the dictionary follows pandas split column format, deserialize into a pandas Dataframe
        input = pd.read_json(json.dumps(input_data),
                             orient="split",
                             dtype=False)
    else:
